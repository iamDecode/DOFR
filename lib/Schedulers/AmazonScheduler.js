        ...
        return new Promise(function(resolve, reject){
            VMManager.getVMs().then(function(VMs) {
                ...
                Promise.settle(promises).then(function(queues){
                    ...
                    //Find smallest queue
                    var bestQueue = queues.reduce(function(a, b){ return AmazonScheduler.lowest(a, b); });
                    var worstQueue = queues.reduce(function(a, b){ return AmazonScheduler.highest(a, b); });
                    ...
                    if(AmazonScheduler.queueLength(bestQueue) > config.get('queue.maxSize')) {
                        VMManager.createVM();
                    } 
                    //Remove worst VM when queueLength subceeds threshold
                    else if(AmazonScheduler.queueLength(worstQueue) < config.get('queue.minSize') && VMs.length > 1) {
                    ...
                    var uuid = AmazonScheduler.uuidFromQueue(bestQueue);
                    this.createTask({uuid: uuid}, taskData).then(resolve);
